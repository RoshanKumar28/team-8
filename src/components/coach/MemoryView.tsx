...
import { downloadDataUrl, downloadText, getReportFile } from "@/lib/files";
      ...
      <Section icon={FlaskConical} title="Your reports, in plain words">
        ...
        {memory.reports.length > 0 && (
          <ul className="mt-3 space-y-1.5 border-t border-line pt-2.5">
            {memory.reports.map((r, i) => {
              const f = r.fileKey ? getReportFile(r.fileKey) : null;
              return (
                <li key={i} className="flex items-center justify-between gap-2 text-[12px]">
                  <span className="min-w-0 truncate text-muted">
                    📄 {r.name}{r.takenOn ? ` · ${r.takenOn}` : ""}
                  </span>
                  {f ? (
                    <button
                      onClick={() => downloadDataUrl(f.dataUrl, f.name)}
                      className="shrink-0 rounded-full border border-line bg-surface px-2.5 py-1 text-[10.5px] font-bold text-brand"
                    >
                      ↓ Download
                    </button>
                  ) : (
                    <span className="shrink-0 text-[10px] italic text-faint">original not kept</span>
                  )}
                </li>
              );
            })}
        ...
        <button
          onClick={() => downloadText(doctorSummary(memory), "ovy-doctor-summary.txt")}
          className="card-soft mt-3 w-full rounded-[var(--r-sm)] bg-brand py-2.5 text-[12px] font-bold text-brandink"
        >
          ↓ Download summary for your doctor
        </button>
        <p className="mt-1.5 text-center text-[10px] text-faint">
          Everything above, one page — hand it over at the start of the visit.
        </p>
      </Section>
...
function doctorSummary(m: Memory): string {
  const p = m.profile;
  const L: string[] = [];
  L.push("PATIENT SUMMARY — prepared with ovy (self-tracked; not a diagnosis)");
  L.push(`Date: ${new Date().toLocaleDateString()}`);
  L.push("");
  if (p.name) L.push(`Name: ${p.name}${p.age ? `, ${p.age}` : ""}`);
  L.push(`PCOS status: ${p.diagnosed === "yes" ? "diagnosed" : p.diagnosed === "no" ? "not diagnosed" : "not yet evaluated"}`);
  if (p.primaryConcern) L.push(`Main concern: ${p.primaryConcern}${p.primaryConcernWhy ? ` — "${p.primaryConcernWhy}"` : ""}`);
  if (p.symptoms.length) L.push(`Reported symptoms: ${p.symptoms.join(", ")}`);
  L.push("");
  L.push(`Cycles: ${[p.cycleLength, p.cycleRegularity, p.lastPeriod && `last period ${p.lastPeriod}`].filter(Boolean).join(" · ") || "not recorded"}`);
  if ((m.periodDates ?? []).length) L.push(`Logged period days: ${m.periodDates.join(", ")}`);
  L.push("");
  L.push("ROTTERDAM CRITERIA (patient-tracked evidence):");
  L.push(`- Irregular/absent ovulation: ${m.criteria.irregularCycles.state} ${m.criteria.irregularCycles.evidence}`);
  L.push(`- Hyperandrogenism signs: ${m.criteria.highAndrogen.state} ${m.criteria.highAndrogen.evidence}`);
  L.push(`- Ovarian morphology: ${m.criteria.ovarianMorphology.state} ${m.criteria.ovarianMorphology.evidence}`);
  L.push("");
  if (m.labs.length) {
    L.push("LAB VALUES ON FILE:");
    for (const l of m.labs) L.push(`- ${l.marker}: ${l.value} (ref ${l.refRange}) [${l.flag}]`);
    L.push("");
  }
  if (p.meds.length || m.medications.length) {
    L.push(`Medications/supplements: ${[...p.meds, ...m.medications.map((x) => x.name)].join(", ")}`);
  }
  if (p.tried.length) L.push(`Previously tried: ${p.tried.join(", ")}`);
  if (m.leadingIndicators.length) {
    L.push("");
    L.push("RECENT TRENDS (self-reported):");
    for (const li of m.leadingIndicators) L.push(`- ${li.name}: ${li.baseline} -> ${li.current} (${li.trend})`);
  }
  L.push("");
  L.push("Generated by ovy. Self-tracked data for discussion — not medical advice.");
  return L.join("\n");
}
